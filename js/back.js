    ...
    $.fn.gantt = function (options) {
        ...
        var core = {
            ...
            leftPanel: function (element) {
                /* Left panel */
                var ganttLeftPanel = $('<div class="leftPanel" style="'+(settings.leftPanelWidth === undefined ? '' :  'width:' + settings.leftPanelWidth + 'px')+'"/>')
                    .append($('<div class="row spacer"/>')
                    .css("height", tools.getCellSize() * (element.headerRows-1))); // 预留出tableHeader的高度
                    
                // 创建grid头部
                var headerEntries = [];
                $.each(settings.gridHeaders, function(i, header){
                    if(i === 0){
                        headerEntries.push(
                            '<td class="name header-name">'+
                                '<span class="fn-label">分组</span>'+
                            '</td>'
                        );
                    }
                    headerEntries.push(
                        '<td class="desc header-desc header-desc-'+header.field+'" >'+
                            '<span class="fn-label" style="'+(header.width === undefined ? "" : "width:"+header.width+"px")+'">'+
                            (header.label || '')+
                            '</span>'+
                        '</td>'
                    );
                ...
                ganttLeftPanel.append($('<table class="leftPanel-grid"><tr class="row header">'+headerEntries.join("")+'</tr></table>'));
                            ...
                            // entry.desc: map
                             if (entry.desc) {
                                 rowEntries.push(
                                    ...
                                    '</span>' +
                                    '</div>');
                                 $.each(settings.gridHeaders, function(j, gridHeader){
                                     var key = gridHeader.field;
                                      rowEntries.push(
                                      '<td class="cell desc row' + i + '-cell cell' + key +
                                      ' " id="RowdId_' + i + dataId + + '_' + key +'">' +
                                      '<span class="fn-label">' +
                                      (entry.desc[key] || '') +
                                      '</span>' +
                                      '</td>');
                ...
                ganttLeftPanel.find('.leftPanel-grid').append(entries.join(""));
